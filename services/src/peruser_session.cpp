...
int32_t PerUserSession::RemoveClient(
    const sptr<IInputClient> &client, bool isUnbindFromClient, bool isInactiveClient, bool isNotifyClientAsync)
{
    if (client == nullptr) {
        return ErrorCode::ERROR_CLIENT_NULL_POINTER;
    }
...
int32_t PerUserSession::OnSetCallingWindow(uint32_t callingWindowId, sptr<IInputClient> client)
{
    ...
    if (clientInfo->config.windowId != callingWindowId) {
        ...
        auto curDisplayId = clientInfo->config.inputAttribute.callingWindowDisplayId;
        if (curDisplayId != callingWindowInfo.displayId_) {
            clientInfo->config.inputAttribute.callingWindowDisplayId =  callingWindowInfo.displayId_ ;
    ...
    const int32_t callingPid, const uint64_t displayId)
{
    ...
    if (clientInfo->config.inputAttribute.callingWindowDisplayId < 0) {
        IMSA_HILOGD("callingWindowDisplayId invaild");
        return;
    }
    ...
    if (clientInfo->config.windowId == windowId) {
        uint64_t curDisplay = clientInfo->config.inputAttribute.callingWindowDisplayId;
        if (curDisplay != displayId) {
            clientInfo->config.inputAttribute.callingWindowDisplayId = displayId;
