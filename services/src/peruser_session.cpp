...
namespace MiscServices {
        ...
        while (1) {
            ...
            switch (msg->msgId_) {
                ...
                case MSG_ID_CLIENT_DIED: {
                    auto *remoteObject = reinterpret_cast<IRemoteObject *>(msg->msgContent_->ReadPointer());
                    if (remoteObject == nullptr) {
                        IMSA_HILOGE("MSG_ID_CLIENT_DIED remoteObject is nullptr");
                        break;
                    }
                    OnClientDied(remoteObject);
                    break;
                }
                case MSG_ID_IMS_DIED: {
                    auto *remoteObject = reinterpret_cast<IRemoteObject *>(msg->msgContent_->ReadPointer());
                    if (remoteObject == nullptr) {
                        IMSA_HILOGE("MSG_ID_IMS_DIED remoteObject is nullptr");
                        break;
                    }
                    OnImsDied(remoteObject);
                    break;
                }
    ...
    */
    void PerUserSession::ResetIme(InputMethodProperty *defaultIme, InputMethodProperty *securityIme)
    {
        ...
        for (int i = 0; i < MIN_IME; i++) {
            ...
            bool flag = false;
            for (auto mapClient : mapClients) {
                if ((i == DEFAULT_IME && !mapClient.second->attribute.GetSecurityFlag())
                    || (i == SECURITY_IME && mapClient.second->attribute.GetSecurityFlag())) {
                    flag = true;
                    break;
                }
            }
    ...
    */
    int PerUserSession::AddClient(const ClientInfo &clientInfo)
    {
        ...
        if (info != nullptr) {
            IMSA_HILOGE("info is exist, not need add.");
        ...
        if (info == nullptr) {
            IMSA_HILOGE("failed to create ClientInfo");
    ...
    */
    ...
    {
        IMSA_HILOGI("PerUserSession::RemoveClient");
    ...
    */
    ...
    \param remoteObject the remote object handler of the input client died.
    */
    void PerUserSession::OnClientDied(IRemoteObject *remoteObject)
    {
        IMSA_HILOGI("PerUserSession::OnClientDied Start...[%{public}d]\n", userId_);
        auto it = mapClients.find(remoteObject);
        if (it == mapClients.end()) {
            IMSA_HILOGE("PerUserSession::RemoveClient client not found");
            return;
        }
        ...
        if (client->AsObject().GetRefPtr() == remoteObject) {
    ...
    \param remoteObject the remote object handler of input method service remoteObject died.
    */
    void PerUserSession::OnImsDied(IRemoteObject *remoteObject)
    {
        (void)remoteObject;
    ...
    */
    void PerUserSession::OnUserLocked()
    {
        ...
        for (auto it = mapClients.begin(); it != mapClients.end();) {
            ...
            ClientInfo *clientInfo = it->second;
            if (clientInfo != nullptr) {
                object->RemoveDeathRecipient(clientInfo->deathRecipient);
    ...
    */
    ClientInfo *PerUserSession::GetClientInfo(const sptr<IInputClient> &inputClient)
    {
        ...
        sptr<IRemoteObject> object = Platform::RemoteBrokerToObject(inputClient);
        auto it = mapClients.find(object);
        if (it == mapClients.end()) {
    ...
    */
    void PerUserSession::OnPrepareInput(Message *msg)
    {
        ...
        sptr<RemoteObjectDeathRecipient> clientDeathRecipient = new (std::nothrow)
            RemoteObjectDeathRecipient(Utils::GetUserId(), MSG_ID_CLIENT_DIED);
        if (clientDeathRecipient == nullptr) {
            IMSA_HILOGE("failed to create clientDeathRecipient");
        }

        int ret = AddClient({ pid, uid, userId_, displayId, client, channel, *attribute, clientDeathRecipient });
        delete attribute;
        if (ret != ErrorCode::NO_ERROR) {
