...
public:
    ...
    uint64_t GetDisplayIdByPid(int64_t callingPid, int32_t userId, sptr<IRemoteObject> abilityToken = nullptr) override;
    bool IsValidVirtualIme(int32_t callingUid) override;
    bool IsSpecialSaUid() override;
    bool IsUIExtension(int64_t pid) override;

private:
    std::pair<bool, FocusedInfo> IsFocusedUIAbility(int64_t callingPid, uint32_t windowId,
        const std::vector<Rosen::FocusChangeInfo> &focusWindowInfos, int32_t userId);
    std::pair<bool, FocusedInfo> IsFocusedUIAbility(int64_t callingPid, uint64_t displayId,
        const std::vector<Rosen::FocusChangeInfo> &focusWindowInfos, int32_t userId);
    ...
    std::pair<bool, FocusedInfo> IsFocusedUIExtension(uint32_t callingTokenId, const sptr<IRemoteObject> &abilityToken,
        const std::vector<Rosen::FocusChangeInfo> &focusWindowInfos, int32_t userId);
    std::pair<bool, FocusedInfo> IsFocusedUIExtension(uint32_t windowId, uint64_t displayId,
        const std::vector<Rosen::FocusChangeInfo> &focusWindowInfos, int32_t userId);
    std::pair<bool, FocusedInfo> IsFocusedUIExtension(uint64_t displayId, uint32_t callingTokenId,
        const std::vector<Rosen::FocusChangeInfo> &focusWindowInfos, int32_t userId);
    bool IsBrokerInner(Security::AccessToken::AccessTokenID tokenId);
    std::pair<bool, FocusedInfo> IsFocusedScbNotEnable(int64_t callingPid, uint32_t callingTokenId, uint32_t windowId,
        const sptr<IRemoteObject> &abilityToken, int32_t userId);
    bool IsFocusedUIExtension(uint32_t callingTokenId, sptr<IRemoteObject> abilityToken, int32_t userId);
