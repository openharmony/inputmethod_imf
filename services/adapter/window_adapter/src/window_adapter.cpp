...
int32_t WindowAdapter::GetDisplayGroupId(uint32_t windowId, int32_t userId, uint64_t &displayGroupId)
{
    IMSA_HILOGD("by windowId run in:%{public}d.", windowId);
    auto displayId = GetDisplayIdByWindowId(windowId, userId);
    return GetDisplayGroupIdWithRetry(displayId, userId, displayGroupId);
}
// LCOV_EXCL_STOP
