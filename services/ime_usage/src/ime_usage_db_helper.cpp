...
// Duration columns in DB, mapped to their screen status codes
...
const std::vector<DurationColumnInfo> DURATION_COLUMNS = {
    { FIELD_UNFOLDED_PORTRAIT_DURATION, UNFOLDED_PORTRAIT },
    { FIELD_UNFOLDED_LANDSCAPE_DURATION, UNFOLDED_LANDSCAPE },
    { FIELD_FOLD_PORTRAIT_DURATION, FOLD_PORTRAIT },
    { FIELD_FOLD_LANDSCAPE_DURATION, FOLD_LANDSCAPE },
    { FIELD_EXPAND_PORTRAIT_DURATION, EXPAND_PORTRAIT },
    { FIELD_EXPAND_LANDSCAPE_DURATION, EXPAND_LANDSCAPE },
    { FIELD_G_PORTRAIT_DURATION, G_PORTRAIT },
    { FIELD_G_LANDSCAPE_DURATION, G_LANDSCAPE },
    { FIELD_N_PORTRAIT_DURATION, N_PORTRAIT },
    { FIELD_N_LANDSCAPE_DURATION, N_LANDSCAPE },
    { FIELD_LM_PORTRAIT_DURATION, LM_PORTRAIT },
    { FIELD_LM_LANDSCAPE_DURATION, LM_LANDSCAPE },
};
 
// Map screen status code -> setter on ImeUsageInfo
using InfoUpdater = std::function<void(ImeUsageInfo &, uint32_t)>;
std::map<int32_t, InfoUpdater> InitUpdateHandlers()
{
    return {
        { UNFOLDED_PORTRAIT, [](ImeUsageInfo &i, uint32_t t) { i.unFoldedPortraitDuration += t; } },
        { UNFOLDED_LANDSCAPE, [](ImeUsageInfo &i, uint32_t t) { i.unFoldedLandscapeDuration += t; } },
        { FOLD_PORTRAIT, [](ImeUsageInfo &i, uint32_t t) { i.foldPortraitDuration += t; } },
        { FOLD_LANDSCAPE, [](ImeUsageInfo &i, uint32_t t) { i.foldLandscapeDuration += t; } },
        { EXPAND_PORTRAIT, [](ImeUsageInfo &i, uint32_t t) { i.expandPortraitDuration += t; } },
        { EXPAND_LANDSCAPE, [](ImeUsageInfo &i, uint32_t t) { i.expandLandscapeDuration += t; } },
        { G_PORTRAIT, [](ImeUsageInfo &i, uint32_t t) { i.gPortraitDuration += t; } },
        { G_LANDSCAPE, [](ImeUsageInfo &i, uint32_t t) { i.gLandscapeDuration += t; } },
        { N_PORTRAIT, [](ImeUsageInfo &i, uint32_t t) { i.nPortraitDuration += t; } },
        { N_LANDSCAPE, [](ImeUsageInfo &i, uint32_t t) { i.nLandscapeDuration += t; } },
        { LM_PORTRAIT, [](ImeUsageInfo &i, uint32_t t) { i.lmPortraitDuration += t; } },
        { LM_LANDSCAPE, [](ImeUsageInfo &i, uint32_t t) { i.lmLandscapeDuration += t; } },
    };
}
 
void UpdateUsageInfo(ImeUsageInfo &info, uint32_t time, int32_t screenStatus)
{
    static auto handlers = InitUpdateHandlers();
    auto it = handlers.find(screenStatus);
    if (it != handlers.end()) {
        it->second(info, time);
    }
}
...
void CollectForegroundEvents(std::shared_ptr<NativeRdb::ResultSet> resultSet,
    const std::string &targetBundle, std::vector<ForegroundRawEvt> &events)
{
...
void CalculateForegroundDuration(const std::vector<ForegroundRawEvt> &events,
    uint64_t startTime, uint64_t endTime, int32_t screenStatus, ImeUsageInfo &info)
{
    if (events.empty()) {
        ...
        UpdateUsageInfo(info, duration, screenStatus);
        return;
    }
    ...
    if (oldest.rawId == ImeUsageEventId::EVENT_INPUT_STATUS_CHANGED) {
        ...
        UpdateUsageInfo(info, duration, oldest.screenBefore);
    }
    for (int i = static_cast<int>(events.size()) - 1; i > 0; i--) {
        ...
        if (tsDelta > 0 && htDelta > 0 &&
            std::abs(tsDelta - htDelta) > DIVERGENCE_THRESHOLD_MS) {
            IMSA_HILOGW("CalculateForegroundDuration: ts/happenTime divergence at idx=%{public}d, "
                "tsDelta=%{public}lld, htDelta=%{public}lld", i,
                static_cast<long long>(tsDelta), static_cast<long long>(htDelta));
        }
        uint32_t duration = (events[i - 1].ts > events[i].ts)
                                ? static_cast<uint32_t>(events[i - 1].ts - events[i].ts)
                                : 0;
        UpdateUsageInfo(info, duration, events[i].screenAfter);
    }
    auto &newest = events[0];
    uint32_t duration = static_cast<uint32_t>(endTime - newest.happenTime);
    UpdateUsageInfo(info, duration, newest.screenAfter);
}
    ...
    while ((pos = path.find('/', pos + 1)) != std::string::npos) {
        ...
        if (access(subPath.c_str(), F_OK) != 0) {
            if (mkdir(subPath.c_str(), S_IRWXU | S_IRWXG) != 0 && errno != EEXIST) {
                IMSA_HILOGE("EnsureDirectoryExist: mkdir %{public}s failed, errno=%{public}d",
                    subPath.c_str(), errno);
                return false;
            }
        }
...
bool ImeUsageDbHelper::VerifyAndRecoverStore(const std::string &dbFile,
    NativeRdb::RdbStoreConfig &config, int &errCode)
{
    ...
    if (checkCount < 0) {
        ...
        if (tableRet != DB_SUCC) {
            IMSA_HILOGE("VerifyAndRecoverStore: CreateReportStateTable failed "
                "after recovery, ret=%{public}d", tableRet);
            return false;
        }
    }
    return true;
...
{
    std::string sql = "CREATE TABLE IF NOT EXISTS " + std::string(IME_USAGE_DB_TABLE) + " (" +
        std::string(FIELD_ID) + " INTEGER PRIMARY KEY AUTOINCREMENT, " +
        std::string(FIELD_RAWID) + " INTEGER NOT NULL, " +
        std::string(FIELD_TS) + " INTEGER NOT NULL, " +
        std::string(FIELD_FOLD_STATUS) + " INTEGER, " +
        std::string(FIELD_PRE_FOLD_STATUS) + " INTEGER, " +
        std::string(FIELD_BUNDLE_NAME) + " TEXT NOT NULL, " +
    ...
    rdbStore_->ExecuteSql("CREATE INDEX IF NOT EXISTS idx_bundle_rawid ON " +
        std::string(IME_USAGE_DB_TABLE) + "(bundle_name, rawid)");
    rdbStore_->ExecuteSql("CREATE INDEX IF NOT EXISTS idx_happen_time ON " +
        std::string(IME_USAGE_DB_TABLE) + "(happen_time)");
    return DB_SUCC;
}
...
int64_t ImeUsageDbHelper::GenerateTimestamp()
{
...
int ImeUsageDbHelper::AddEvent(const ImeEventRecord &record, const DurationMap &durations)
{
    ...
    if (ret != DB_SUCC) {
        IMSA_HILOGE("Insert failed, ret=%{public}d, rawId=%{public}d, bundle=%{public}s",
    ...
    IMSA_HILOGD("AddEvent: rowId=%{public}lld, rawId=%{public}d, bundle=%{public}s, "
        ...
        static_cast<long long>(rowId), record.rawid, record.bundleName.c_str(),
        record.screenStatus, static_cast<long long>(record.happenTime), durations.size());
...
int ImeUsageDbHelper::QueryRawEventIndex(const std::string &bundleName, int32_t rawId)
{
    ...
    if (resultSet == nullptr) {
        IMSA_HILOGI("QueryRawEventIndex: resultSet is null, bundle=%{public}s, rawId=%{public}d",
            bundleName.c_str(), rawId);
        return -1;
    }
    int32_t id = -1;
    ...
    resultSet->Close();
    IMSA_HILOGD("QueryRawEventIndex: bundle=%{public}s, rawId=%{public}d, foundId=%{public}d",
        bundleName.c_str(), rawId, id);
    return id;
}
 
void ImeUsageDbHelper::QueryEventRecords(int32_t startIndex, int64_t dayStartTime,
    const std::string &bundleName, std::vector<ImeEventRecord> &records)
{
    std::lock_guard<std::mutex> lock(dbMutex_);
    if (rdbStore_ == nullptr) {
        return;
    ...
    std::string sql = "SELECT id, rawid, ts, happen_time, bundle_name, fold_status, "
        "pre_fold_status FROM " + std::string(IME_USAGE_DB_TABLE) +
        " WHERE bundle_name = ? AND id >= ? AND happen_time >= ? ORDER BY id ASC";
    auto resultSet = rdbStore_->QuerySql(sql,
        std::vector<std::string>{ bundleName, std::to_string(startIndex), std::to_string(dayStartTime) });
    if (resultSet == nullptr) {
    ...
    IMSA_HILOGD("QueryEventRecords: bundle=%{public}s, startIndex=%{public}d, "
        ...
        bundleName.c_str(), startIndex, static_cast<long long>(dayStartTime),
        records.size(), checkpointCount);
}
 
void ImeUsageDbHelper::QueryStatisticEventsInPeriod(uint64_t startTime, uint64_t endTime,
    std::unordered_map<std::string, ImeUsageInfo> &infos)
{
    ...
    auto resultSet = rdbStore_->QuerySql(sql,
        std::vector<std::string>{ std::to_string(ImeUsageEventId::EVENT_INPUT_START),
          std::to_string(static_cast<int64_t>(startTime)),
    ...
    while (resultSet->GoToNextRow() == DB_SUCC) {
        ...
        for (const auto &dc : DURATION_COLUMNS) {
            ...
            UpdateUsageInfo(info, static_cast<uint64_t>(std::max<int64_t>(0, val)), dc.screenStatus);
            colIdx++;
        }
        int64_t startNum = 0;
...
void ImeUsageDbHelper::QueryFinalEventInfo(uint64_t endTime, ImeUsageRawEvent &event)
{
    ...
    std::string sql = "SELECT id, rawid, bundle_name, ts, happen_time, fold_status, "
        "pre_fold_status FROM " + std::string(IME_USAGE_DB_TABLE) +
        " WHERE happen_time <= ? ORDER BY id DESC LIMIT 1";
    auto resultSet = rdbStore_->QuerySql(sql,
        std::vector<std::string>{ std::to_string(static_cast<int64_t>(endTime)) });
    if (resultSet == nullptr) {
    ...
    if (resultSet->GoToNextRow() == DB_SUCC) {
        ...
        IMSA_HILOGD("QueryFinalEventInfo: id=%{public}lld, rawId=%{public}d, "
            ...
            static_cast<long long>(event.id), event.rawId,
            event.package.c_str(),
            event.screenStatusAfter, event.screenStatusBefore);
    } else {
...
void ImeUsageDbHelper::QueryForegroundImeInfo(uint64_t startTime, uint64_t endTime, int32_t screenStatus,
    ImeUsageInfo &info)
{
    std::lock_guard<std::mutex> lock(dbMutex_);
    if (rdbStore_ == nullptr) {
    ...
    IMSA_HILOGD("QueryForegroundImeInfo: startTime=%{public}llu, endTime=%{public}llu, "
        ...
        static_cast<unsigned long long>(startTime), static_cast<unsigned long long>(endTime),
        screenStatus, info.package.c_str());
...
int ImeUsageDbHelper::DeleteEventsByTime(uint64_t clearDataTime)
{
    ...
    std::string sql = "DELETE FROM " + std::string(IME_USAGE_DB_TABLE) +
        " WHERE happen_time < ?";
    ...
    IMSA_HILOGI("Deleted rows older than %{public}llu",
...
int ImeUsageDbHelper::SaveReportState(const std::string &key, const std::string &value)
{
    ...
    int64_t rowId = -1;
    int ret = rdbStore_->InsertWithConflictResolution(rowId, IME_USAGE_STATE_TABLE, values,
...
std::vector<uint64_t> ImeUsageDbHelper::QueryActiveDays(uint64_t startTime, uint64_t endTime)
{
    ...
    auto resultSet = rdbStore_->QuerySql(sql,
        std::vector<std::string>{ std::to_string(static_cast<int64_t>(startTime)),
            std::to_string(static_cast<int64_t>(endTime)) });
    if (resultSet == nullptr) {
        return days;
    }
    ...
    resultSet->Close();
    IMSA_HILOGI("QueryActiveDays: found %{public}zu days in [%{public}llu, %{public}llu]",
        days.size(), static_cast<unsigned long long>(startTime),
        static_cast<unsigned long long>(endTime));
    return days;
}
} // namespace MiscServices
