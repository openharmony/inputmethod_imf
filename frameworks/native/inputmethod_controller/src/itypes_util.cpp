...
{
    if (!Marshal(data, input.pid, input.uid, input.userID, input.displayID, input.isShowKeyboard, input.eventFlag,
...
{
    if (!Unmarshal(
            data, output.pid, output.uid, output.userID, output.displayID, output.isShowKeyboard, output.eventFlag)) {
