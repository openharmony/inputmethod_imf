...
private:
    ...
    int32_t HideKeyboard(Trigger trigger, uint32_t sessionId, int32_t clientSessionId = -1);
    ...
    int32_t HidePanel(const std::shared_ptr<InputMethodPanel> &inputMethodPanel, PanelFlag flag, Trigger trigger,
        uint32_t sessionId, int32_t clientSessionId = -1);
    ...
    int32_t HideKeyboardImplWithoutLock(int32_t cmdId, uint32_t sessionId, int32_t clientSessionId = -1);
