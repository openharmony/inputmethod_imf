...
napi_value JsGetInputMethodSetting::Init(napi_env env, napi_value exports)
{
    ...
    NAPI_CALL(napi_create_int32(env, MAX_TYPE_NUM, &maxTypeNumber));
    ...
    NAPI_CALL(napi_define_class(env, IMS_CLASS_NAME.c_str(), IMS_CLASS_NAME.size(), JsConstructor, nullptr,
                       sizeof(properties) / sizeof(napi_property_descriptor), properties, &cons));
    NAPI_CALL(napi_create_reference(env, cons, 1, &IMSRef_));
    NAPI_CALL(napi_set_named_property(env, exports, IMS_CLASS_NAME.c_str(), cons));
    return exports;
}
