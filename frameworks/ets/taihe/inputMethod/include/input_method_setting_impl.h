...
public:
    ...
    taihe::array<InputMethodProperty_t> GetInputMethodSync(bool enable);
...
private:
    ...
    std::mutex mutex_;
    std::map<std::string, std::vector<CallbackObject>> jsCbMap_;
};
...
public:
    IMFSettingImpl() {}
    taihe::array<InputMethodProperty_t> GetInputMethodSync(bool enable) {
        return InputMethodSettingImpl::GetInstance().GetInputMethodSync(enable);
    }
    taihe::array<InputMethodSubtype_t> ListCurrentInputMethodSubtypeSync() {
    ...
    void OnImeChange(taihe::callback_view<void(InputMethodProperty_t const&, InputMethodSubtype_t const&)> f, uintptr_t opq) {
