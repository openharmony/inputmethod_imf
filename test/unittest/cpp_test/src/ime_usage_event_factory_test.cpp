...
void ImeUsageEventFactoryTest::SetUp()
{
    ...
    factory_ = std::make_unique<ImeUsageEventFactory>(dataHelper_);
}
...
// After settlement, only COUNT_DURATION remains (START/STOP are deleted)
static void InsertSession(ImeUsageDataHelper &db, uint64_t dayStart, const SessionParams &params)
{
    ...
    durations[ScreenStatusToIndex(params.screenStatus)] = static_cast<uint64_t>(stopHappenTime - startHappenTime);
 ...
 * @tc.desc: Constructor with valid dataHelper
 * @tc.type: FUNC
 */
HWTEST_F(ImeUsageEventFactoryTest, Constructor_001, TestSize.Level0)
{
    ASSERT_NE(factory_, nullptr);
    // Verify dataHelper_ is stored from constructor argument
    EXPECT_NE(factory_->dataHelper_, nullptr);
    // Verify dataHelper_ is the same instance passed in constructor
    EXPECT_EQ(factory_->dataHelper_, dataHelper_);
    // Verify dataHelper is usable
    EXPECT_TRUE(factory_->dataHelper_->IsReady());
}

/**
 * @tc.name: ImeUsageEventFactory_Constructor_002
 * @tc.desc: Constructor with nullptr dataHelper
 * @tc.type: FUNC
 */
HWTEST_F(ImeUsageEventFactoryTest, Constructor_002, TestSize.Level0)
{
    ...
    ASSERT_NE(f, nullptr);
    EXPECT_EQ(f->dataHelper_, nullptr);
}
 ...
 * @tc.desc: Create with nullptr dataHelper returns empty infos
 * @tc.type: FUNC
 */
...
HWTEST_F(ImeUsageEventFactoryTest, Create_003, TestSize.Level0)
{
    ...
    InsertSession(*dataHelper_, dayStart, { TEST_BUNDLE, 3600000, 7200000, UNFOLDED_PORTRAIT });
...
HWTEST_F(ImeUsageEventFactoryTest, GetUsageInfo_002, TestSize.Level0)
{
    uint64_t dayStart = DayStartFromMs(GetToday0ClockMs());
    InsertSession(*dataHelper_, dayStart, { TEST_BUNDLE, 3600000, 7200000, UNFOLDED_PORTRAIT });
    InsertSession(*dataHelper_, dayStart, { TEST_BUNDLE2, 10800000, 14400000, EXPAND_PORTRAIT });
...
// ==================== MergeForegroundInfo: zero foreground duration ====================

/**
 * @tc.name: ImeUsageEventFactory_MergeForegroundInfo_006
 * @tc.desc: MergeForegroundInfo with START at exactly endTime produces zero foreground duration
 * @tc.type: FUNC
 */
HWTEST_F(ImeUsageEventFactoryTest, MergeForegroundInfo_006, TestSize.Level0)
{
    ...
    // Insert START at exactly dayEnd — foreground duration will be 0
    ImeEventRecord startRec;
    startRec.rawid = EVENT_INPUT_START;
    startRec.ts = static_cast<int64_t>(dayEnd);
    startRec.happenTime = static_cast<int64_t>(dayEnd);
    startRec.bundleName = TEST_BUNDLE;
    startRec.screenStatus = UNFOLDED_PORTRAIT;
    startRec.preScreenStatus = UNFOLDED_PORTRAIT;
    dataHelper_->AddEvent(startRec);
    ...
    std::unordered_map<std::string, ImeUsageInfo> statisticInfos;
    factory_->MergeForegroundInfo(statisticInfos, dayStart, dayEnd);
    // Should have added TEST_BUNDLE but with 0 usage (duration = endTime - happenTime = 0)
    auto it = statisticInfos.find(TEST_BUNDLE);
    ASSERT_NE(it, statisticInfos.end());
    EXPECT_EQ(it->second.usage, 0u);
}

// ==================== MergeForegroundInfo: STATUS_CHANGED without START (cross-midnight) ====================

/**
 * @tc.name: ImeUsageEventFactory_MergeForegroundInfo_007
 * @tc.desc: MergeForegroundInfo with only STATUS_CHANGED (no START) uses screenBefore for oldest segment
 * @tc.type: FUNC
 */
HWTEST_F(ImeUsageEventFactoryTest, MergeForegroundInfo_007, TestSize.Level0)
{
    uint64_t dayStart = DayStartFromMs(GetToday0ClockMs());
    uint64_t dayEnd = dayStart + MILLISECS_PER_DAY - 1;

    // Insert only STATUS_CHANGED (cross-midnight: START happened before dayStart)
    ImeEventRecord changedRec;
    changedRec.rawid = EVENT_INPUT_STATUS_CHANGED;
    changedRec.ts = dayStart + 3600000;
    changedRec.happenTime = dayStart + 3600000;
    changedRec.bundleName = TEST_BUNDLE;
    changedRec.screenStatus = EXPAND_PORTRAIT;      // screenAfter
    changedRec.preScreenStatus = UNFOLDED_PORTRAIT; // screenBefore
    dataHelper_->AddEvent(changedRec);

    std::unordered_map<std::string, ImeUsageInfo> statisticInfos;
    factory_->MergeForegroundInfo(statisticInfos, dayStart, dayEnd);
    // Should add foreground duration with both screenBefore and screenAfter segments
    auto it = statisticInfos.find(TEST_BUNDLE);
    ASSERT_NE(it, statisticInfos.end());
    // dayStart → STATUS_CHANGED: uses screenBefore (UNFOLDED_PORTRAIT)
    EXPECT_GT(it->second.durations[IDX_UNFOLDED_PORTRAIT], 0u);
    // STATUS_CHANGED → dayEnd: uses screenAfter (EXPAND_PORTRAIT)
    EXPECT_GT(it->second.durations[IDX_EXPAND_PORTRAIT], 0u);
}

// ==================== Branch Coverage: MergeForegroundInfo null dataHelper ====================

/**
 * @tc.name: ImeUsageEventFactory_MergeForegroundInfo_NullDataHelper
 * @tc.desc: MergeForegroundInfo with null dataHelper_ does not crash and statisticInfos unchanged
 * @tc.type: FUNC
 */
HWTEST_F(ImeUsageEventFactoryTest, MergeForegroundInfo_NullDataHelper, TestSize.Level0)
{
    // Construct factory with nullptr dataHelper
    auto factory = std::make_unique<ImeUsageEventFactory>(nullptr);
    ASSERT_NE(factory, nullptr);

    std::unordered_map<std::string, ImeUsageInfo> statisticInfos;
    // Pre-populate with one entry to verify it remains unchanged
    ImeUsageInfo info;
    info.package = TEST_BUNDLE;
    info.durations[IDX_UNFOLDED_PORTRAIT] = 100;
    info.usage = 100;
    statisticInfos[TEST_BUNDLE] = info;

    uint64_t dayStart = GetToday0ClockMs();
    uint64_t dayEnd = dayStart + MILLISECS_PER_DAY - 1;
    // Should not crash; returns immediately due to null dataHelper_
    factory->MergeForegroundInfo(statisticInfos, dayStart, dayEnd);

    // statisticInfos should be unchanged
    ASSERT_EQ(statisticInfos.size(), 1u);
    auto it = statisticInfos.find(TEST_BUNDLE);
    ASSERT_NE(it, statisticInfos.end());
    EXPECT_EQ(it->second.durations[IDX_UNFOLDED_PORTRAIT], 100u);
}
